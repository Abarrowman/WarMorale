...
void main() {
  ...
  geom_out.tex_cord = vec2(tl_texture_corner.x + char_width / texture_width, tl_texture_corner.y);
  EmitVertex();
  ...
  geom_out.tex_cord = vec2(tl_texture_corner.x + char_width / texture_width, tl_texture_corner.y + float(char_height) / texture_height);
  EmitVertex();
}